...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_accordeon__WEBPACK_IMPORTED_MODULE_6__["default"])('.accordion-heading', '.accordion-block');
...
var accordeon = function accordeon(triggersSelector, itemsSelector) {
  var btns = document.querySelectorAll(triggersSelector); // textBlocks = document.querySelectorAll(itemsSelector);
  // 1
...
var filter = function filter() {
  ...
  menu.addEventListener('click', function (evt) {
    ...
    if (target && target.tagName == 'LI') {
      ...
      }); // target - элемент на который кликнул пользователь, содержится в target
  ...
  upload.forEach(function (elem) {
    elem.addEventListener('input', function () {
      ...
      if (elem.files[0].size > maxSize) {
        alert("File size > then ".concat(maxSize));
        resetInputs();
      }
    });
  });
...
var scrolling = function scrolling(upSelector) {
  ...
  var calcScroll = function calcScroll() {
    upElem.addEventListener('click', function (evt) {
      ...
      if (this.hash !== '') {
        evt.preventDefault();
        var hashElementByScroll = document.querySelector(this.hash),
            hashElementTop = 0;
...
var showCards = function showCards(triggerSelector, wrapperSelector) {
...
var sliders = function sliders(slides, direction, prev, next) {
  ...
  try {
    ...
    nextBrn.addEventListener('click', function () {
      plusSlides(1);
